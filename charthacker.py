...
# ── color pairs ────────────────────────────────────────────────────────────────
# Bull/bear candle colors are split into fixed, never-redefined pairs per
# scheme (rather than one pair number whose RGB gets swapped via init_pair on
# toggle) so that switching schemes is an ordinary content change — redrawn
# correctly and instantly by both real ncurses (Termux) and windows-curses/
# PDCurses, which — unlike ncurses — doesn't reliably force a full repaint
# when an existing pair's underlying color is redefined. See bull_pair()/
# bear_pair()/vol_bull_pair()/vol_bear_pair() below for the scheme lookup.
C_BULL_BW     = 1   # bullish, blue/white scheme  → white
C_BEAR_BW     = 2   # bearish, blue/white scheme  → blue
C_AXIS      = 3
C_LABEL     = 4
C_HEADER    = 5
C_VOL_BULL_BW = 6   # bullish volume, blue/white scheme → white
C_VOL_BEAR_BW = 7   # bearish volume, blue/white scheme → blue
C_BULL_RG     = 38  # bullish, red/green scheme  → green
C_BEAR_RG     = 39  # bearish, red/green scheme  → red
C_VOL_BULL_RG = 40  # bullish volume, red/green scheme → green
C_VOL_BEAR_RG = 41  # bearish volume, red/green scheme → red
C_ASSET_SEL = 9
...
# ── colors ────────────────────────────────────────────────────────────────────
def bull_pair():
    return C_BULL_RG if state.color_scheme == "rg" else C_BULL_BW

def bear_pair():
    return C_BEAR_RG if state.color_scheme == "rg" else C_BEAR_BW

def vol_bull_pair():
    return C_VOL_BULL_RG if state.color_scheme == "rg" else C_VOL_BULL_BW

def vol_bear_pair():
    return C_VOL_BEAR_RG if state.color_scheme == "rg" else C_VOL_BEAR_BW

def init_colors(scheme: str = "bw"):
    """Sets up ALL candle color pairs (both schemes) once, fixed forever —
    bull_pair()/bear_pair()/vol_bull_pair()/vol_bear_pair() pick which pair
    number to use at draw time based on state.color_scheme. `scheme` param
    kept for call-site compatibility but no longer changes anything here."""
    curses.start_color()
    curses.use_default_colors()
    curses.init_pair(C_BULL_BW,     curses.COLOR_WHITE,  -1)
    curses.init_pair(C_BEAR_BW,     curses.COLOR_BLUE,   -1)
    curses.init_pair(C_VOL_BULL_BW, curses.COLOR_WHITE,  -1)
    curses.init_pair(C_VOL_BEAR_BW, curses.COLOR_BLUE,   -1)
    curses.init_pair(C_BULL_RG,     curses.COLOR_GREEN,  -1)
    curses.init_pair(C_BEAR_RG,     curses.COLOR_RED,    -1)
    curses.init_pair(C_VOL_BULL_RG, curses.COLOR_GREEN,  -1)
    curses.init_pair(C_VOL_BEAR_RG, curses.COLOR_RED,    -1)
...
def _save_prompt_dialog(stdscr, rows: int, cols: int):
    """
    Ask 'Save chart before exit?'
    Returns:  True  = yes, save and quit
              False = no, quit without saving
              None  = Esc / cancel (don't quit at all)
    """
    msg  = " Save chart before exit? "
    hint = " [Y] Save & quit   [N] Quit without saving   [Esc] Stay "
    box_w = max(len(msg), len(hint)) + 4
    box_h = 5
    ...
    for r in range(box_h):
        try:
            stdscr.addstr(box_y + r, box_x, " " * min(box_w, cols - box_x),
                          curses.color_pair(C_CURSOR) | curses.A_BOLD)
        except curses.error:
            pass
    try:
        stdscr.addstr(box_y,     box_x, msg.center(box_w)[:cols - box_x],
                      curses.color_pair(C_ALERT) | curses.A_BOLD | curses.A_REVERSE)
        stdscr.addstr(box_y + 2, box_x, hint.center(box_w)[:cols - box_x],
    ...
    while True:
        ...
        if k in (ord("n"), ord("N")):
            stdscr.timeout(50)
            return False
        if k == 27:  # Esc = cancel quit, stay in chart
            stdscr.timeout(50)
            return None


def save_chart_state() -> bool:
    """
    Persist user drawings (hlines), position tools, and manually-created alerts
    to SAVE_FILE (chart_state.json). Trade-monitor alerts (auto-generated from
    Phemex positions) are excluded — they recreate themselves on next launch.
    Returns True on success.
    """
    with state.lock:
        hlines_snap    = list(state.hlines)
        pos_tools_snap = list(state.pos_tools)
    ...
    def _clean_postool(pt):
        """Strip candle-index fields that become stale across sessions.
        _anchor_ts is kept — it's a Unix timestamp and is always valid."""
        skip = {"_anchor_idx", "_stop_idx"}
        return {k: v for k, v in pt.items() if k not in skip}

    def _clean_alert(a):
        """Strip volatile runtime keys; keep _trade_id for reference."""
        skip = {"_last_state", "_fired"}
        return {k: v for k, v in a.items() if k not in skip}

    # Only save user-created alerts (no _trade_id → not from trade monitor)
    user_alerts = [_clean_alert(a) for a in alerts_snap
                   if not a.get("_trade_id")]
    ...
    try:
        ...
        with state.lock:
            state.error = f"Save failed: {e}"
        return False


def load_chart_state():
    """
    Restore hlines, pos_tools, and alerts from SAVE_FILE on startup.
    Silently skips if the file does not exist or is malformed.
    """
    ...
    # Restore runtime-only keys that were stripped on save.
    # _anchor_idx is set to -1 (unresolved) — draw_pos_tools will resolve
    # it on first draw by searching all_candles for _anchor_ts.
    for pt in pos_tools:
        pt["_anchor_idx"] = -1       # lazy-resolve by timestamp on first draw
        pt["_stop_idx"]   = None
        pt.setdefault("active", True)
        pt.setdefault("create_alerts", False)   # alerts already restored below
    ...
    with state.lock:
        state.hlines    = hlines
        state.pos_tools = pos_tools
        state.alerts    = alerts

    with state.lock:
        n_h  = len(state.hlines)
        n_pt = len(state.pos_tools)
        n_al = len(state.alerts)
    if n_h or n_pt or n_al:
        with state.lock:
            state.error = (f"Chart restored: {n_h} line(s)  "
...
def draw(win, db: DoubleBuffer, rows: int, cols: int):
    ...
    if not visible and error:
        db.puts(2, 2, f"ERR: {error}", bear_pair(), curses.A_BOLD)
    elif not visible and status not in ("Live",):
        db.puts(2, 2, status, bear_pair(), curses.A_BOLD)
    ...
    if chart_mode == "line":
        ...
        # Candlestick mode — cvd.py style: solid █ bodies, dim │ wicks, ─ dojis.
        # The full high→low wick is drawn first in a neutral/dim colour, then the
        # solid open→close body is painted over it, so only the body carries the
        # bull/bear colour (matching cvd.py's draw_candles).
        for i, candle in enumerate(visible):
            col = start_col + i
            if not (0 <= col < chart_r):
                continue
            ...
            body_pair   = C_CURSOR if is_selected else (bull_pair() if bull else bear_pair())
    ...
    for i, candle in enumerate(visible):
        ...
        pair        = C_CURSOR if is_selected else \
                      (vol_bull_pair() if candle.c >= candle.o else vol_bear_pair())
    ...
    if loading or not g_ts:
        ...
        if gerr and not loading:
            db.puts(chart_top + chart_h // 2 + 1,
                    ...
                    gerr[:cols - 2], bear_pair(), curses.A_BOLD)
...
def draw_pos_tools(db, chart_top, chart_bot, chart_r, p2r,
    ...
    for pt in pos_tools:
        ...
        # ── Lazy anchor resolution (loaded from disk) ─────────────────────────
        # After a save/load, _anchor_idx is -1 and _anchor_ts holds the Unix
        # timestamp of the original anchor candle.  Search all_candles for the
        # first candle at or after that timestamp and cache the result so this
        # only runs once per tool.
        if anchor_ci < 0:
            anchor_ts = pt.get("_anchor_ts", 0)
            if anchor_ts and all_candles:
                resolved = len(all_candles) - 1   # fallback: rightmost candle
            ...
            pt["_anchor_idx"] = anchor_ci   # cache so next frame is instant
...
def draw_hlines(db, chart_top, chart_bot, chart_r, p2r, hlines):
    ...
    PRICE_W = 12   # must match main draw() layout constant
    for hl in hlines:
        ...
        db.put(row, chart_r, "+", C_VWAP, curses.A_BOLD)
        db.puts(row, chart_r + 1,
                f"{lbl_text:<{PRICE_W}}"[:PRICE_W],
...
def check_open_position_on_startup():
    ...
    try:
        ...
        for p in positions:
            ...
            with state.lock:
                # Don't add duplicate pos_tools
                already = any(t.get("_from_monitor") for t in state.pos_tools)
                if not already:
                    state.pos_tools.append(pt)

                # Don't add duplicate alerts — check by _trade_id (same key
                # format used by trade_monitor_loop so the two systems dedup
                # against each other and never create double TP/SL alerts).
                _existing_tids = {a.get("_trade_id") for a in state.alerts}
...
def main(stdscr):
    ...
    # Restore saved drawings / alerts before any threads start
    load_chart_state()
    ...
    while True:
        ...
        for key in keys:
            ...
            if key in (ord("q"), ord("Q")):
                ...
                if _save_choice is None:
                    # Esc — user cancelled the quit; restore non-blocking mode
                    db.prev = None
                    continue
                if _save_choice:
                    save_chart_state()
            ...
            if key in (ord("e"), ord("E")):
                ...
                # bull_pair()/bear_pair()/etc. resolve to a different FIXED pair
                # number per scheme (see init_colors()), so this is now an
                # ordinary content change — redraws correctly and instantly on
                # both ncurses (Termux) and windows-curses/PDCurses without any
                # forced full-repaint trick.
                ...
                if _ac:
                    ...
                    if _pt:
                        ...
                        _pt["_anchor_ts"]  = _ac.ts   # persisted across sessions
                ...
                # [U] = save chart state (drawings, pos tools, alerts) to disk
                ok = save_chart_state()
                with state.lock:
