...
class ChartState:
    def __init__(self):
        ...
        self.liq_mode         = False
        self.liq_data         = {}    # {price_level: notional_usd}
        self.liq_loading      = False
        self.liq_asset        = "ETH" # ETH or BTC
        self.liq_scroll       = 0     # vertical pan (rows)
        self.liq_session      = 0
        self.global_data      = {}
        self.global_ts        = []
...
def draw(win, db: DoubleBuffer, rows: int, cols: int):
    ...
    # In liquidation mode, render the heatmap view
    if liq_mode:
        draw_liq(db, rows, cols)
        return
...
# ─────────────────────────────────────────────────────────────────────────────
# LIQUIDATION MODE
# Estimates where leveraged positions would be liquidated across the price range.
#
# Method (mirrors CoinGlass Model 1 / Symbol mode):
#   1. Fetch current price + open interest from Phemex public API
#   2. Build a synthetic position distribution: assume positions opened at every
#      price level over the last 12 hours, weighted by volume at that level
#   3. For each leverage tier (5x, 10x, 20x, 25x, 50x, 100x), calculate the
#      liquidation price as: entry * (1 ± 1/(leverage * maintenance_margin))
#   4. Accumulate notional exposure (USD + coin) at each liquidation price bucket
#   5. Repeat for both longs (liq below entry) and shorts (liq above entry)
#
# The result is a heatmap: brighter/redder = more estimated liquidations there
# ─────────────────────────────────────────────────────────────────────────────

LIQ_LEVERAGES   = [5, 10, 20, 25, 50, 100]
LIQ_MAINT_RATE  = 0.005   # 0.5% maintenance margin (Phemex standard)
LIQ_THRESHOLD   = 1.0     # colour threshold multiplier (1.0 = CoinGlass default)
LIQ_BUCKETS     = 200     # price buckets in the heatmap
LIQ_RANGE_PCT   = 0.15    # ±15% price range shown


def fetch_liq_data(asset: str) -> dict:
    """
    Fetch 12h 1-minute klines for `asset` and build liquidation exposure map.
    Returns tuple (exposure_dict, lo, hi, cur_price, bucket_sz) or None on error.
    """
    import os as _os
    _log = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "liq_debug.log")
    def _dbg(msg):
        try:
    ...
    sym = PHEMEX_SYMBOLS.get(asset, "ETHUSDT")
    _dbg(f"fetch_liq_data start: asset={asset} sym={sym}")

    # ── Fetch 1-minute klines for last 12h using /kline/list (from/to) ────────
    # /kline/last has a max limit of 500; /kline/list uses timestamps instead
    import time as _tm
    _now  = int(_tm.time())
    _from = _now - 12 * 3600   # 12 hours ago
    _rows = []
    # Fetch in two 500-candle batches to cover 720 minutes
    for _batch_from, _batch_to in [
        (_from,           _from + 500 * 60),
        (_from + 500*60,  _now),
    ]:
        try:
            _path  = "/exchange/public/md/v2/kline/list"
            _query = (f"symbol={sym}&resolution=60"
                      f"&from={_batch_from}&to={_batch_to}&limit=500")
            _url   = f"{PHEMEX_REST_URL}{_path}?{_query}"
            _dbg(f"GET {_url}")
            _r     = requests.get(_url, headers=_phemex_headers(_path, _query),
                                  timeout=15)
            _dbg(f"HTTP {_r.status_code}  len={len(_r.text)}")
            _dbg(f"Body[:200]: {_r.text[:200]}")
            _json  = _r.json()
            _batch = (_json.get("data") or {}).get("rows", [])
            _dbg(f"batch rows: {len(_batch)}")
            _rows.extend(_batch)
        except Exception as _fe:
            _dbg(f"EXCEPTION: {_fe}")
            with state.lock:
                state.error = f"Liq fetch: {_fe}"
            return None

    _dbg(f"total rows: {len(_rows)}")
    if not _rows:
        with state.lock:
            state.error = f"Liq: no data for {sym}"
        return None

    # ── Build volume-weighted price distribution ───────────────────────────────
    # Each candle: row = [ts, interval, open, high, low, close, vol, turnover]
    candle_data = []
    for row in _rows:
        try:
            ...
            vol = float(row[6])
            vwap_c = (h + l + c) / 3.0
            candle_data.append((vwap_c, vol, h, l))
        except Exception:
            continue
    ...
    cur_price = candle_data[-1][0]

    # Price range for buckets
    lo_price  = cur_price * (1 - LIQ_RANGE_PCT)
    hi_price  = cur_price * (1 + LIQ_RANGE_PCT)
    bucket_sz = (hi_price - lo_price) / LIQ_BUCKETS

    # exposure[bucket_idx] = notional USD
    exposure = [0.0] * (LIQ_BUCKETS + 2)

    def _bucket(price):
        idx = int((price - lo_price) / bucket_sz)
        return max(0, min(LIQ_BUCKETS - 1, idx))

    # For each candle's VWAP, distribute its volume across leverage tiers
    for vwap_c, vol, h, l in candle_data:
        notional = vwap_c * vol  # USD notional at this candle
        if notional <= 0:
            continue

        for lev in LIQ_LEVERAGES:
            # maintenance margin determines liquidation distance
            maint  = LIQ_MAINT_RATE
            # Long liquidation: entry * (1 - 1/leverage + maint)
            # Short liquidation: entry * (1 + 1/leverage - maint)
            liq_long  = vwap_c * (1.0 - (1.0 / lev) + maint)
            liq_short = vwap_c * (1.0 + (1.0 / lev) - maint)

            # Weight by leverage tier (higher leverage = more clustered exposure)
            weight = notional / len(LIQ_LEVERAGES)

            if lo_price <= liq_long <= hi_price:
                exposure[_bucket(liq_long)]  += weight
            if lo_price <= liq_short <= hi_price:
                exposure[_bucket(liq_short)] += weight

    # Convert to {price: notional} dict
    result = {}
    for i, exp in enumerate(exposure[:LIQ_BUCKETS]):
        if exp > 0:
            price = lo_price + (i + 0.5) * bucket_sz
            result[price] = exp

    return result, lo_price, hi_price, cur_price, bucket_sz


def load_liq_data(session: int):
    """Background loader for liquidation mode."""
    with state.lock:
        state.liq_loading = True
        asset = state.liq_asset
    try:
        result = fetch_liq_data(asset)
    except Exception:
        result = None
    with state.lock:
        if state.liq_session == session:
            ...
            state.liq_loading = False  # always clear loading flag


# ── Color mapping for heatmap intensity ───────────────────────────────────────
# Map normalized exposure (0-1) → curses color pair
# We reuse existing color pairs: dim=low, bright=high
# Intensity bands: 0-20% dim blue, 20-40% cyan, 40-60% yellow, 60-80% magenta, 80-100% red

def _liq_intensity_pair(norm: float) -> tuple:
    """Return (color_pair, attrs) for a given normalized [0-1] exposure."""
    if norm < 0.20:
        return C_AXIS, curses.A_DIM
    elif norm < 0.40:
        return C_BTD_BUY, curses.A_NORMAL      # cyan
    elif norm < 0.60:
        return C_VWAP_SD2, curses.A_BOLD       # yellow
    elif norm < 0.80:
        return C_BTD_SELL, curses.A_NORMAL     # magenta-adjacent
    else:
        return C_SESS_EXCL, curses.A_BOLD      # red = hottest


def draw_liq(db: "DoubleBuffer", rows: int, cols: int):
    """
    Render the Liquidation Heatmap mode.
    Layout:
      Row 0:   Header (title, asset selector, controls)
      Row 1:   Subheader (price range info, threshold)
      Rows 2…n-3: Heatmap rows (price levels, intensity bars, notional labels)
                  + candlestick chart overlaid in the right portion
      Row n-2: Current price line
      Row n-1: Footer
    """
    with state.lock:
        liq_data    = dict(state.liq_data)
        loading     = state.liq_loading
        asset       = state.liq_asset
        liq_scroll  = state.liq_scroll
        cur_price   = state.last_price

    HEADER_H  = 2
    FOOTER_H  = 1
    PRICE_COL = 12   # width of price label on left
    BAR_W     = 30   # width of exposure bar
    INFO_W    = 22   # notional info on right
    chart_top = HEADER_H
    chart_bot = rows - FOOTER_H - 1
    chart_h   = max(1, chart_bot - chart_top)
    footer_row = rows - 1
    ...
    for c in range(cols):
        db.put(0, c, " ", C_HEADER)
        db.put(1, c, " ", C_HEADER)
    db.puts(0, 2, "Q U A N T A S S E T  |  ChartHacker  —  LIQUIDATIONS",
            C_HEADER, curses.A_BOLD)
    # Asset selector
    for _a in ("ETH", "BTC"):
        _lbl  = f" {_a} "
        _pair = C_ASSET_SEL if _a == asset else C_HEADER
        db.puts(0, cols - 20 + (0 if _a == "ETH" else 6), _lbl, _pair, curses.A_BOLD)
    db.puts(1, 2,
            f"  12H Liq Map  Threshold:{LIQ_THRESHOLD:.1f}  "
            f"[E]TH [B]TC  [↑↓] scroll  [R]efresh  [L]iq exit  [P]shot  [Q]uit",
            C_LABEL, curses.A_NORMAL)

    if loading or not liq_data:
        msg = "  Loading liquidation data..." if loading else "  No data — press [R] to load"
        db.puts(chart_top + chart_h // 2, 2, msg, C_AXIS, curses.A_BOLD)
        with state.lock:
            _le = state.error
        _lf = " LIQ  [E]TH [B]TC  [R] refresh  [L] exit"
        if _le:
            _lf += f"  | {_le}"
        db.puts(footer_row, 0, _lf.ljust(cols)[:cols],
                C_ASSET_SEL, curses.A_BOLD)
        return

    exposure  = liq_data.get("exposure", {})
    lo_price  = liq_data.get("lo",  cur_price * 0.85)
    hi_price  = liq_data.get("hi",  cur_price * 1.15)
    bucket_sz = liq_data.get("bucket", (hi_price - lo_price) / LIQ_BUCKETS)

    if not exposure:
        db.puts(chart_top + 2, 2, "  No exposure data in range", C_AXIS, curses.A_DIM)
        return

    # ── Compute display range with scroll ─────────────────────────────────────
    # Each row shows one bucket. Chart_h rows visible.
    # Sort prices high→low (top = highest price like CoinGlass)
    sorted_prices = sorted(exposure.keys(), reverse=True)
    n_buckets     = len(sorted_prices)
    max_scroll    = max(0, n_buckets - chart_h)
    liq_scroll    = max(0, min(liq_scroll, max_scroll))

    # Update state scroll clamp
    with state.lock:
        state.liq_scroll = liq_scroll

    visible_prices = sorted_prices[liq_scroll: liq_scroll + chart_h]

    # Normalize exposure for color mapping
    max_exp  = max(exposure.values()) if exposure else 1.0
    # Apply threshold: anything below threshold% of max is suppressed
    threshold_val = max_exp * LIQ_THRESHOLD * 0.1   # scale threshold

    # ── Draw heatmap rows ─────────────────────────────────────────────────────
    for ri, price in enumerate(visible_prices):
        row     = chart_top + ri
        if row >= chart_bot:
            break
        exp     = exposure[price]
        norm    = exp / max_exp
        pair, attrs = _liq_intensity_pair(norm)

        # Skip below threshold
        if exp < threshold_val and norm < 0.05:
            db.puts(row, 0, f" {price:>10.2f}", C_AXIS, curses.A_DIM)
            continue

        # Price label (left)
        price_lbl = f" {price:>10.2f}"
        db.puts(row, 0, price_lbl, pair, attrs)

        # Exposure bar
        bar_len = max(1, int(norm * BAR_W))
        bar_str = "█" * bar_len + "░" * (BAR_W - bar_len)
        try:
            for ci, ch in enumerate(bar_str):
                if PRICE_COL + ci < cols - INFO_W:
                    db.put(row, PRICE_COL + ci, ch, pair, attrs)
        except Exception:
            pass

        # Notional labels
        notional_usd   = exp
        notional_coin  = exp / price if price > 0 else 0
        coin_sym       = asset
        if notional_usd >= 1_000_000:
            usd_lbl = f"${notional_usd/1_000_000:.1f}M"
        elif notional_usd >= 1_000:
            usd_lbl = f"${notional_usd/1_000:.0f}K"
        else:
            usd_lbl = f"${notional_usd:.0f}"

        if notional_coin >= 1000:
            coin_lbl = f"{notional_coin/1000:.1f}K {coin_sym}"
        else:
            coin_lbl = f"{notional_coin:.1f} {coin_sym}"

        info_str = f" {usd_lbl:>8} {coin_lbl:>10}"
        db.puts(row, cols - INFO_W, info_str[:INFO_W], pair, attrs)

        # Current price indicator
        if abs(price - cur_price) < bucket_sz * 1.5:
            db.puts(row, PRICE_COL + BAR_W + 1, f"◄ ${cur_price:.2f}",
                    C_ALERT, curses.A_BOLD | curses.A_REVERSE)

    # ── Footer ────────────────────────────────────────────────────────────────
    n_lev_str = "+".join(str(l) for l in LIQ_LEVERAGES)
    footer = (f" LIQ  {asset}/USDT  "
              f"Range:{lo_price:.0f}-{hi_price:.0f}  "
              f"Levers:{n_lev_str}x  "
              f"Showing {min(chart_h,n_buckets)}/{n_buckets} levels  "
              f"Scroll:{liq_scroll}  "
              f"[↑↓] pan  [R] refresh  [L] exit")
    db.puts(footer_row, 0, footer.ljust(cols)[:cols], C_ASSET_SEL, curses.A_BOLD)
...
def main(stdscr):
    ...
    while True:
        ...
        for key in keys:
            ...
            new_asset = None
            new_feed  = None
            # E/B: in liq mode switches liq asset; otherwise switches chart asset
            with state.lock:
                _in_liq2 = state.liq_mode
            if key in (ord("e"), ord("E")):
                if _in_liq2:
                    with state.lock:
                        state.liq_asset   = "ETH"
                        state.liq_session += 1
                        _ls2 = state.liq_session
                        state.liq_loading = True
                    threading.Thread(target=load_liq_data,
                                     args=(_ls2,), daemon=True).start()
                elif state.asset != "ETH":
                    new_asset = "ETH"
                ...
                # L: exit liq mode > close hline list > toggle line chart
                with state.lock:
                    _hl_open  = state.show_hline_list
                    _liq_on_l = state.liq_mode
                if _liq_on_l:
                    with state.lock:
                        state.liq_mode = False
                    db.prev = None
                elif _hl_open:
                    with state.lock:
                ...
                # [;] = toggle Liquidation mode
                with state.lock:
                    state.liq_mode = not state.liq_mode
                    _enter_liq  = state.liq_mode
                    state.liq_session += 1
                    _liq_sess   = state.liq_session
                    state.liq_loading = True
                if _enter_liq:
                    threading.Thread(target=load_liq_data,
                                     args=(_liq_sess,), daemon=True).start()
                db.prev = None
            elif key in (ord("r"), ord("R")):
                with state.lock:
                    _in_global = state.global_mode
                    _in_econ   = state.show_econ_cal
                    _in_liq_r  = state.liq_mode
                    _g_sess    = state.session
                if _in_liq_r:
                    with state.lock:
                        state.liq_session += 1
                        _lsess_r = state.liq_session
                        state.liq_loading = True
                    threading.Thread(target=load_liq_data,
                                     args=(_lsess_r,), daemon=True).start()
                elif _in_global:
                    start_global(_g_sess)
                elif _in_econ:
                    threading.Thread(target=load_econ_calendar, daemon=True).start()
