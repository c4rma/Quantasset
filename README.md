...
**Bias glyph color now shows Absorption vs. Clean, not just direction (2026-09-04 user-reported — circling ▼ markers on the chart and asking why VE's own risk structure took no partial-close action on them):** the glyph's color used to encode bias direction only (green/red/dim), completely identical whether the bar was classified Absorption or Clean — the one distinction VE's own partial-close logic actually cares about (it only ever reacts to Absorption, correctly ignoring Clean). A red ▼ could mean either, indistinguishable at a glance, which is exactly what made a genuinely-correct "VE took no action" read as a bug. Color now matches the bottom VolEffort histogram's own already-established convention (magenta = Absorption, cyan = Clean) instead of bias; shape (▲/▼/◆) still shows the bias direction, so both dimensions are visible from one glyph.
...
**Sessions overlay + vertical pan added (2026-09-03 user request — "Include sessions in the NDX OHLC chart" + "give it vertical & horizontal scrolling, the same as is in QQQ and ETH"):** the NDO/Morning/Lunch/Power Hour/EOD/EEOD colored session bands are ported verbatim from `_draw_ohlc_chart_panel`'s own SESSIONS OVERLAY (same table, same per-day border/label/time-axis-strip logic, same shared `[O]` show/hide toggle) — no ETH-specific 19:00 CT period separator, since that marks crypto's own trading-day rollover, not a meaningful boundary for an equity index. `[↑]`/`[↓]`/`[J]` vertical pan now works too, using the same shift-the-window-before-p2r approach the main panel's own `ohlc_vert_offset` uses, sourced from NDX's own state slot (horizontal scroll and crosshair already worked as of the previous fix — this closes the one remaining gap against full ETH/QQQ parity for these controls).
...
**Sim trades are shown as a retroactive "1R+W" hypothetical (2026-08-27 explicit user request):** every sim trade's real entry/exit prices are untouched, but Qty/Sequence/Gross/Fees/Funding/Net PnL/Balance/DD/R$ and the PnL chart are all recomputed as if Aggressive's win-boost progression had been active for the entire trade history, sized off today's live PCT/RISK_DOLLARS setting walked forward against the hypothetical balance. R:R comes back numerically identical to the real trade's own R:R — a uniform position-size rescale can't change a reward:risk ratio, so this is expected, not a bug. DVOL Layer 1 and the drawdown ladder are NOT replayed on top of this (no historical record of either to replay from). Real trades are unaffected — always the actual historical record.

**Historical NV-Auto "1R+0.33W" trades are exempt from the hypothetical (2026-09-04 user-reported — "The balance shown in Data and the one shown in the main dashboard are not the same"):** NV-Auto briefly (2026-09-02 to 2026-09-04) had its own inherent "1R+0.33W" progression that overrode `SIZING_MODE` entirely, so the hypothetical's own *different* boost formula (100%, not 33%) on top of an already-real compounding history no longer answered any "what would Standard have missed" question — it just silently diverged the table's own running balance from the real one, which is what surfaced as this exact report. Any row whose own real `sequence_label` starts with `"1R+0.33W"` (captured before the hypothetical ever runs) still passes through completely untouched — real qty/gross/fees/net/balance, walked forward by its own real net PnL — while every other trade still gets the original hypothetical treatment unchanged. NV-Auto's inherent progression itself has since been removed (see Sizing Modes below), so no new trade will ever produce this label going forward; the skip logic stays only to keep replaying those specific historical rows correctly.

A **"Live sizing mode: Standard/Aggressive"** readout (2026-08-27, right below the title) shows the REAL, currently-active sizing mode regardless of which source tab is open, and regardless of trading mode — added after a user report ("the win progression did not kick in") traced back to this exact table: the hypothetical always shows "Aggressive"/boosted sequence labels, which had been mistaken for confirmation that live Aggressive mode + boosting was actually running, when the live mode was genuinely Standard the whole time (Standard never applies any progression, by design). Toggle the real mode with `[0]`. The PnL chart and Net PnL figures ARE already fee-adjusted net, not gross — verified directly against the stats header's own "Total" figure, which is mathematically the chart's own final (not peak) point on the cumulative curve.
...
**NV-Auto** (2026-09-02 user request) is a fourth trading mode, `[9]`-cycled after NV: `Order Flow → BTD → NV → NV-Auto → Order Flow`. It shares every rule NV has (Net-Volume-derived per-asset regime, same target/sizing/risk-management pipeline) with one change to *when* an entry fires: instead of waiting for the next BTD-style candle-close confirmation, NV-Auto enters **immediately** the instant Net Volume goes Positive or Negative, capped at one entry attempt per 1-minute bar (see the fix just below — it originally had no such cap at all).

**One-entry-per-candle fix (2026-09-04 user-reported, trade-table screenshot showing eight ETH entries inside two consecutive 1-minute bars, most stopped out in 1-38 seconds before immediately re-entering — "Athena entered several trades back-to-back all in the same candle. This should not happen. Only 1 trade may be opened per candle."):** the original design skipped the "once per new bar" dedup entirely, reasoning that becoming ARMED at all under NV-Auto already requires a decisive regime, so a successful order placement moving the instrument out of WATCHING/ARMED would naturally stop further attempts. That reasoning only covers the *first* entry after a genuine flip — it breaks the moment that first entry gets stopped out fast: the SL close sets state back to WATCHING immediately, and since Net Volume itself hasn't actually flipped, the very next cycle re-arms and re-enters with nothing to stop it, repeatedly, all within the same still-forming bar — exactly the reported whipsaw. Now uses the same one-attempt-per-bar dedup (`_last_checked_bar_ts`) BTD/NV already rely on for their own candle-close confirmation. A second flip within the same minute simply waits for the next bar to roll over (≤60s) rather than re-entering immediately — the deliberate trade-off for "only 1 trade may be opened per candle."
...
**Sizing under NV-Auto** (2026-09-04 user-reported — "The 'Aggressive' sizing mode has been active all this time, but for some reason Athena has been using 'Aggressive/1R+0.33W' instead somehow? ... Whichever sizing mode the user has set in the main dashboard should be the same one used in Data."): NV-Auto briefly (2026-09-02 to 2026-09-04) had its own inherent "1R+0.33W" progression, checked ahead of and completely overriding `SIZING_MODE`, regardless of which `[0]` Sizing Mode was active. This has been **removed** — `[0]`'s SIZING_MODE (Standard/Aggressive/Aggressive-1R+0.33W) now governs sizing under NV-Auto exactly like it does under every other trading mode (Order Flow/BTD/NV), through the exact same shared `_compute_trade_risk_dollars`/`_update_sizing_progression` code path. See [Sizing Modes](#sizing-modes) below.
...
These three modes govern sizing under **every** trading mode uniformly, including NV-Auto (2026-09-04 user-reported/decided — see Trading Engine's own NV-Auto Confirmation section). NV-Auto briefly had its own separate, inherent "1R+0.33W" progression that overrode `SIZING_MODE` entirely regardless of which mode was selected here; it has since been removed so that `[0]` is the single source of truth for sizing everywhere.
...
**BT/ST anchoring fix (2026-09-03 user-reported, screenshot of the live Deribit chain — "Athena shows BT at $2520 for ETH, but it should be $2420... when price moves far in either direction, BT/ST keep moving up strikes even though they should not"):** `compute_bt_st` scans the option chain for the nearest-to-spot run of consecutive strikes where one side's volume dominates, then used to anchor BT/ST at the edge of that *minimal* run closest to spot. Once price sits well inside an already-large, uniformly one-sided zone — confirmed live: ETH calls dominated every strike from $2420 clear through $3000 (a 15-strike block) while spot climbed from ~2420 to ~2515 — that "nearest edge" trivially collapses to wherever spot currently is, sliding BT up right along with every price tick instead of staying pinned to the zone's own real boundary. Fixed: the found run is now extended out to its full contiguous same-dominance block, then anchored at whichever edge of that block borders a *genuine* flip to the opposite side (not merely the end of the fetched chain, and not a blank/neutral strike) — that boundary is the actual wall, and it only moves when the options market's own positioning actually flips. Verified against the exact reported case (BT/ST now $2420/$2400, not $2520/$2540) and against three synthetic edge cases: a short dominant pocket sandwiched between opposite territory on both sides, and a uniformly one-sided band with no flip anywhere in range, both correctly fall back to the original nearest-to-spot tie-break (unchanged from before this fix) since neither has a real boundary to anchor to.
