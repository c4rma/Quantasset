#!/usr/bin/env python3
"""
Quantasset — Options Chain (crypto via Deribit, equities/ETFs via CBOE)
Curses-based terminal viewer with in-place cell updates (no flicker).
Usage: python chain.py [ETH|BTC|<equity/ETF ticker>] [-s]
  -s   Simple mode: Strike, Vol, OI, Mark only
Press 's' inside the viewer to switch symbols at runtime (crypto or equity).
"""
...
IS_CRYPTO = CURRENCY in ("ETH", "BTC")   # anything else is tried as a CBOE-listed equity/ETF ticker
...
def fetch_live_price(symbol):
    """Best-effort near-real-time quote via Yahoo's v8 chart 'meta' block — CBOE's
    options feed is ~15m delayed, so this is used to keep the header/ATM marker
    from looking stale. Returns None on any failure — caller falls back to CBOE's
    own (delayed) reference price."""
    try:
...
def fetch_all_equity(symbol):
    """Any CBOE-listed, optionable equity/ETF ticker. The OCC option-symbol parsing
    below is offset-from-the-end (date+type+strike are always the trailing 15 chars
    regardless of ticker length), so this works for any ticker CBOE's delayed-quotes
    endpoint recognizes without further changes."""
    ...
    # Mark/last are already USD from CBOE (theo/last_trade_price), so "index_price"
    # for the mark*index_price multiplication draw() does elsewhere is just 1.0 here.
    strikes_data = {}
    for cp_flag, strike, o in by_exp[target_exp]:
        ...
        strikes_data.setdefault(strike, {})[otype] = {
            ...
            "mark_iv":       (o.get("iv") or 0.0) * 100.0,   # CBOE reports iv as a decimal
        }
    ...
    return {
        "index_price":   1.0,
        "display_price": spot,
        "perp_price":    live_price,
        "perp_source":   "Yahoo" if live_price else None,
        "atm_ref_price": spot,
        "expiry_ts":     expiry_ts,
        "strikes_data":  strikes_data,
        "atm_strike":    atm_strike,
...
def fetch_all():
    return fetch_all_crypto() if IS_CRYPTO else fetch_all_equity(CURRENCY)
...
def prompt_symbol(stdscr):
    """Prompt the user on the status line for a new ticker symbol. Returns
    the uppercased symbol string, or None if the user cancelled (Esc/empty)."""
    h, w = stdscr.getmaxyx()
    row = h - 1
    label = "Symbol (e.g. ETH, BTC, SPY, AAPL): "
    ...
    while True:
        stdscr.move(row, 0)
        stdscr.clrtoeol()
        safe_add(stdscr, row, 0, (label + buf).ljust(w - 1)[:w - 1], cp(P_STATUS))
        stdscr.move(row, min(len(label) + len(buf), w - 2))
        stdscr.refresh()
...
def draw(win, data, status=""):
    ...
    instrument_label = f"{CURRENCY}-0DTE" if IS_CRYPTO else CURRENCY

    if SIMPLE and IS_CRYPTO:
        pieces = [
            ("QUANTASSET",                                      cp(P_CYAN, bold=True)),
            ("  │  ",                                           cp(P_DIM, dim=True)),
            (instrument_label,                                  cp(P_DEFAULT, bold=True)),
        ...
        pieces = [
            ...
            (f"${display_price:,.2f}" if display_price else "—", cp(P_YELLOW, bold=True)),
            ("  IV ",                                           cp(P_DIM, dim=True)),
            (f"{atm_iv:.1f}%",                                  cp(P_YELLOW, bold=True)),
            ("  ",                                              cp(P_DIM, dim=True)),
            (ttl,                                               cp(P_YELLOW, bold=True)),
    ...
    hint = f" q=quit  r=refresh  s=symbol  [{CURRENCY}]  [{mode_tag}]  {status}"
...
# ── CURSES MAIN ───────────────────────────────────────────────────────────────
def _loading_msg():
    src = "Deribit" if IS_CRYPTO else "CBOE"
    tag = "0DTE " if IS_CRYPTO else ""
    return f"Fetching {CURRENCY} {tag}chain from {src}…"

def curses_main(stdscr):
    global CURRENCY, IS_CRYPTO
    curses.curs_set(0)
    stdscr.nodelay(True)
    stdscr.timeout(250)
    init_colors()
    ...
    while True:
        ...
        if key in (ord('s'), ord('S')):
            new_symbol = prompt_symbol(stdscr)
            if new_symbol and new_symbol != CURRENCY:
                CURRENCY  = new_symbol
                IS_CRYPTO = CURRENCY in ("ETH", "BTC")
                with lock:
                    data       = None
                    error_msg  = ""
                    last_fetch = 0.0
